...
#NOTA: obtenho as estimativas robustas da média e da matriz de covariância
summary(robustezY1)
...
#Outliers
outAQplotY1 = which(adjquanY1$outliers == TRUE) #Obtenho as observações que são outliers
length(outAQplotY1)
...
#Outliers
outAQplotY2 = which(adjquanY2$outliers == TRUE) #Obtenho as observações que são outliers
length(outAQplotY2)
...
#------------------------------------------------------------------------------#
# Metodologias GRÁFICAS ####
#options(max.print=10000) #Para não omitir linhas dos 'dadosY2'
...
cartPortY2$classe = 0 #Acrescentar a coluna das classes
...
#Construir o mapa de Portugal
base = data.frame(Municipios = portugalY2@data$Concelho, Classe = cartPortY2$Classe) #Data frame do cartograma
head(base) #Verifica-se as classes para cada município
...
#Outliers
outAQplotY3 = which(adjquanY3$outliers == TRUE) #Obtenho as observações que são outliers
length(outAQplotY3)
...
cartPortY3$classe = 0 #Acrescentar a coluna das classes
...
#Construir o mapa de Portugal
base = data.frame(Municipios = portugalY3@data$Concelho, Classe = cartPortY3$Classe) #Data frame do cartograma
head(base) #Verifica-se as classes para cada município
